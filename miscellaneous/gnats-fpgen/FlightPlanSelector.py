...
class FlightPlanSelector:
	...
	def __init__(self,fname = "../../GNATS_Server/share/tg/trx/TRX_DEMO_100rec_beta1.5.trx"):
		self.fname = fname;
		self.readFile();
	...
	def readFlightPlan(self, origin, destination):
		...
		tmp_Sids = terminalAreaInterface.getAllSids(origin);
		...
		tmp_arrivalStars = terminalAreaInterface.getAllStars(destination);
	...
	def getTerminalProcedures(self, origin, destination, departureRunway, arrivalRunway):
		...
		departureSids = terminalAreaInterface.getAllSids(origin)

		arrivalStars = terminalAreaInterface.getAllStars(destination)
		...
		# SID
		enrouteWp = terminalAreaInterface.getWaypoint_Latitude_Longitude_deg(firstEnroutePoint)
		distanceToEnroute = 9999999
		optimalSid = ""
		for sid in departureSids:
			legs = terminalAreaInterface.getProcedure_leg_names('SID', sid, origin)
			for leg in legs:
				legWps = terminalAreaInterface.getWaypoints_in_procedure_leg("SID", sid, origin, leg)
				if legWps:
					latLonVal = terminalAreaInterface.getWaypoint_Latitude_Longitude_deg(list(legWps)[-1])
		...
		approachProcedures = terminalAreaInterface.getAllApproaches(destination)
		...
		result_Procedure_leg_names = terminalAreaInterface.getProcedure_leg_names('APPROACH', optimalApproach, destination);
		if not (result_Procedure_leg_names is None) :
			approachLeg = result_Procedure_leg_names[0]
			firstApproachWaypoint = terminalAreaInterface.getWaypoints_in_procedure_leg("APPROACH", optimalApproach, destination, approachLeg)[0]

		# STAR
		distanceToApproach = 9999999
		approachWp = terminalAreaInterface.getWaypoint_Latitude_Longitude_deg(firstApproachWaypoint)
		optimalStar = ""
		for star in arrivalStars:
			legs = terminalAreaInterface.getProcedure_leg_names('STAR', star, destination)
			for leg in legs:
				legWps = terminalAreaInterface.getWaypoints_in_procedure_leg("STAR", star, destination, leg)
				latLonVal = terminalAreaInterface.getWaypoint_Latitude_Longitude_deg(list(legWps)[-1])
				if not (latLonVal is None) :
	...
	def generateDepartureTaxiPlan(self, origin_airport, departure_runway, origin_gate) :
		...
		tmp_array_node_data = airportInterface.getLayout_node_data(origin_airport);
		...
		if (tmp_node_seq == -1) :
			...
			tmp_array_node_map = airportInterface.getLayout_node_map(origin_airport);
			...
			tmp_design_taxi_plan = airportInterface.get_taxi_route_from_A_To_B("", origin_airport, origin_gate, departing_runway_entry);
	...
	def generateArrivalTaxiPlan(self, destination_airport, arrival_runway, destination_gate) :
		...
		tmp_array_node_data = airportInterface.getLayout_node_data(destination_airport);
		if not(tmp_array_node_data is None) :
			for i in range(0, len(tmp_array_node_data)) :
		...
		if (tmp_node_seq == -1) :
			...
			tmp_array_node_map = airportInterface.getLayout_node_map(destination_airport);
			if not(tmp_array_node_map is None) :
				for i in range(0, len(tmp_array_node_map)) :
					if (tmp_node_seq == tmp_array_node_map[i][1]) :
			...
			tmp_design_taxi_plan = airportInterface.get_taxi_route_from_A_To_B("", destination_airport, landing_runway_end, destination_gate);
			if not(tmp_design_taxi_plan is None) :
				for i in range(0, len(tmp_design_taxi_plan)) :
					if not(i == 0) :
						tmp_surface_plan_string = tmp_surface_plan_string + ", ";
	...
	def generate(self,
		...
		if flight_plan_type == self.FLIGHT_PLAN_TYPE_GATE_TO_GATE :
			...
			tmp_lat_lon = terminalAreaInterface.getWaypoint_Latitude_Longitude_deg(tmp_first_waypoint);
