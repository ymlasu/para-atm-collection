...
def process_raw_data(raw_dir, processed_dir):
    ...
    id_label = pandas.read_csv(os.path.join(raw_dir, 'classes.csv'))
    src_dst = pandas.read_csv(os.path.join(raw_dir, 'edgelist.csv'))
    # elliptic_txs_features.csv has no header, and it has the same order idx with elliptic_txs_classes.csv
    id_time_features = pandas.read_csv(os.path.join(
        raw_dir, 'features.csv'), skiprows=[0], header=None)
    # get oldId_newId
    ...
    id_time_features[:, 2:] = minmax_scale(id_time_features[:, 2:])
...
class EllipticDataset:
    ...
    def process(self):
        ...
        id_time_features = torch.Tensor(numpy.load(
            os.path.join(self.processd_dir, 'id_time_features.npy')))
        id_label = torch.IntTensor(numpy.load(
            os.path.join(self.processd_dir, 'id_label.npy')))
        src_dst_time = torch.IntTensor(numpy.load(
            os.path.join(self.processd_dir, 'src_dst_time.npy')))
        ...
        if self.self_loop:
            if self.reverse_edge:
                ...
                g.edata['timestamp'] = torch.cat(
                    (src_dst_time[:, 2], src_dst_time[:, 2], id_time_features[:, 1].int()))
            else:
        ...
        time_features = id_time_features[:, 2:]
        label = id_label[:, 1]
        g.ndata['label'] = label
        ...
        # to account for empty timestamps
        ts_list = []
        [ts_list.append(int(x)) for x in id_time_features[:,1]]
        # for i in set(ts_list):
        unique_ts = list(dict.fromkeys(ts_list))
        for i in unique_ts:
