...
class MatGRUGate(torch.nn.Module):
    ...
    def reset_parameters(self):
        init.kaiming_normal_(self.W)
        init.kaiming_normal_(self.U)
...
class TopK(torch.nn.Module):
    ...
    def forward(self, node_embs):
        ...
        out = node_embs[topk_indices] * \
            torch.tanh(scores[topk_indices].view(-1, 1))
        # we need to transpose the output
        return out.t()


class EvolveGCNH(nn.Module):
    def __init__(self, in_feats=166, n_hidden=1, num_layers=2, n_classes=7, classifier_hidden=510):
        ...
        self.recurrent_layers.append(MatGRUCell(
    ...
    def reset_parameters(self):
        for gcn_weight in self.gcn_weights_list:
            init.kaiming_normal_(gcn_weight)
    ...
    def forward(self, g_list):
        ...
        # 1x8
        return torch.mean(self.mlp(feature_list[-1]), dim=0).unsqueeze(0)


class EvolveGCNO(nn.Module):
    def __init__(self, in_feats=128/4, n_hidden=128, num_layers=2, n_classes=7, classifier_hidden=170):
        # default parameters follow the official config
    ...
    def forward(self, g_list):
        ...
        # return self.mlp(feature_list[-1]) # 2x8
        try:
            return self.softmax(torch.mean(self.mlp(feature_list[-1]), dim=0).unsqueeze(0))
            #return torch.mean(self.mlp(feature_list[-1]), dim=0).unsqueeze(0)
        except:
            import pdb
            pdb.set_trace()
